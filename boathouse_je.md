...
- `Description` = for lines that share an account across multiple rows (i.e. the 1007 split
  ...
  2026`). For single-occurrence accounts (GST, PST, Tips, Petty Cash, Boat Rental if it's the
  only 3xxx-adjacent to reuse an account... in practice always prefix per-category revenue
  lines since Ice Cream/3007 recurs across days but is single-occurrence within a day — use
  the category label on every revenue line and tender-brand label on every 1007 line; plain
  memo (no prefix) only for GST/PST/Tips/Petty Cash/Discounts).
